...
static Tensor make_rand_int8(int64_t rows, int64_t cols, uint32_t seed = 0) {
  auto t = Tensor::create(make_shape(rows, cols), DType::INT8);
  std::mt19937 rng(seed);
  std::uniform_int_distribution<int> dist(-127, 127);
  int8_t *p = t.data<int8_t>();
  for (int64_t i = 0; i < rows * cols; ++i)
    p[i] = static_cast<int8_t>(dist(rng));
  return t;
}
...
static void BM_GemmNaive_Square(benchmark::State &state) {
  ...
  state.SetBytesProcessed(state.iterations() *
                          static_cast<int64_t>(3 * N * N * sizeof(float)));
}

// ---- gemm_int8_fixed -------------------------------------------------------
//
// INT8 GEMM has the same arithmetic intensity as FP32 GEMM (2*N^3 ops) but
// uses integer multiply-accumulate.  GFLOPS is reported on the same 2*N^3
// basis for direct comparison with the FP32 variants.
// Bytes processed: N^2 bytes for A (INT8) + N^2 bytes for B (INT8)
//                + 4*N^2 bytes for C (FP32 output).

static void BM_GemmInt8_Square(benchmark::State &state) {
  const int64_t N = state.range(0);
  auto A = make_rand_int8(N, N, 5);
  auto B = make_rand_int8(N, N, 6);
  auto C = Tensor::create(make_shape(N, N), DType::FP32);

  QuantParams qp;
  qp.scale_a = 1.0f / 127.0f;
  qp.scale_b = 1.0f / 127.0f;
  qp.scale_c = 1.0f;
  ...
  state.counters["GFLOPS"] = benchmark::Counter(
      flops, benchmark::Counter::kIsRate, benchmark::Counter::kIs1000);
  // 1 byte per A element + 1 byte per B element + 4 bytes per C element
  state.SetBytesProcessed(state.iterations() *
                          static_cast<int64_t>(N * N * (1 + 1 + 4)));
}
...
// ---- Registration ----------------------------------------------------------
// Standard size range per CLAUDE.md: {64, 128, 256, 512, 1024, 2048, 4096} +
// {384, 768}
